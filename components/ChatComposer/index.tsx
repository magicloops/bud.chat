...
export function ChatComposer({
  ...
  const promoteConversation = usePromoteConversation()
  ...
  const { connect, disconnect } = useStreaming({
    ...
    onConversationCreated: (newConversationId) => {
      console.log('Registry pattern: Promoting conversation:', { displayId: conversationId, realId: newConversationId })
      
      // Simple promotion using registry pattern
      if (conversationId) {
        promoteConversation(conversationId, newConversationId)
        
        // Update URL for temp conversations to the real ID
        // For temp conversations (like temp-123), we want to update URL to the real ID
        if (conversationId?.startsWith('temp-')) {
          console.log('🔄 Updating URL from temp conversation to real ID:', { from: conversationId, to: newConversationId })
          window.history.replaceState(null, '', `/${newConversationId}`)
          
          // Clean up temp registry entry after URL change and a brief delay
          // This ensures streaming is complete and URL has changed before cleanup
          setTimeout(() => {
            console.log('🧹 Cleaning up temp registry entry after URL change:', conversationId)
            clearRegistryEntry(conversationId)
          }, 1000) // Wait 1 second to ensure everything is settled
        }
      }
    }
  })
